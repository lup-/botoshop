...
const ITEMS_NAME = 'polls';
const ITEM_NAME = 'poll';
...
module.exports = {
    ...
    async stats(ctx) {
        const db = await getDb(DB_NAME);
        let pollId = ctx.request.body.pollId;
        let stats = await db.collection('pollAnswers').aggregate([
            { $match: {pollId} },
            { $unwind: "$answers" },
            { $project: {
                    question: "$answers.question",
                    answer: {$cond: [{$not: "$answers.answer.text"}, "$answers.answer", "$answers.answer.text"]}
                }
            },
            { $group: {
            ...
            { $group: {"_id": "$question", question: {$first: "$question"}, answers: {$addToSet: {answer: "$answer", count: "$count"}}} }
        ]).toArray();
